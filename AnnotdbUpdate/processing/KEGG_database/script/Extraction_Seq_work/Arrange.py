...
def get_seq(handle):
    ...
    """
    s = handle.read()
    AASEQ = re.findall("AASEQ\s+\d+\s(.*)NTSEQ", s, re.DOTALL)
    if len(AASEQ) == 0:
        AASEQ = ''
    else:
        AASEQ = AASEQ[0]
    ...
    NTSEQ = re.findall("NTSEQ\s+\d+\s(.*)///", s, re.DOTALL)
    if len(NTSEQ) == 0:
        NTSEQ = ''
    else:
        NTSEQ = NTSEQ[0]
            ...
            try:
                try:
                    ...
                    with open('Error1.log', 'a+') as f:
                        b = str(gene_path)
                        print(time.strftime("%Y:%m:%d:%H:%M:%S", time.localtime()), ':', b, file=f)
            except FileNotFoundError:
                with open('No_this_gene.txt', 'a+') as f:
                    a = str(gene_path)
                    # 可以增加下载功能
                    print(time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime()), ':', a, file=f)
        print(img_df.head())
        img_df.to_csv(path + org + '/' + org + '_allGene_ko.xls', sep='\t', index=False)
        # print(img_df)
        print(org + ':arrange over')
